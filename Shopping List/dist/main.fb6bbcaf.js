...
var shoping_form = document.querySelector('.shopping');
var spohhing_input = document.querySelector('.shopping__input');
var spopping_btn = document.querySelector('.shopping__btn');
var list_ul = document.querySelector('.list_ul'); //we need an arr to hpld our state

var items = [];

function handlSubmit(e) {
  e.preventDefault(); //take a value of input

  var name = e.currentTarget.item.value; //если полу пустое, не добавлять в список
  ...
  items.push(item); //clear the input after add info

  e.target.reset(); //this is custom events, это 
...
function restoreFromLocalstorege() {
  ...
  var lsItems = JSON.parse(localStorage.getItem('items')); //?? if we have data items, push to list_ul
...
list_ul.addEventListener('itemsUpDated', mirrorTpLocalStorage); //!!Desc: delete Items 
...
list_ul.addEventListener('click', function (e) {
  var id = parseInt(e.target.value); //if was click to button = delite list_ul
